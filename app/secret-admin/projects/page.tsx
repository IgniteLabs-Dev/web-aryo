            ...
            <div key={p.id} className="p-5 rounded-2xl bg-[#0a1729] border border-slate-800 flex flex-col justify-between">
              <div>
                <div className="flex flex-wrap gap-1.5 mb-2">
                  {p.tags.map((t: string) => (
                    <span key={t} className="text-[10px] px-2 py-0.5 rounded bg-slate-800 text-slate-300 uppercase">
                      {t}
                    </span>
                  ))}
                </div>
                <span className="text-[10px] font-semibold tracking-wider text-brand-yellow uppercase">
                  {p.category}
                </span>
                ...
                <p className="text-[10px] text-slate-500 mt-2 flex items-center gap-1">
                  <ImageIcon className="w-3 h-3" /> {p.gallery?.length || 0} images
                </p>
              </div>
              <div className="flex gap-2 mt-3">
                <button onClick={() => setGalleryProject(p)} className="flex-1 py-2 rounded-lg bg-slate-800 text-slate-300 text-xs">
                  Gallery
                </button>
                <button onClick={() => setEditing(p)} className="px-3 py-2 rounded-lg bg-blue-600/20 text-blue-400 text-xs">
                  <Edit2 className="w-3 h-3" />
                </button>
                <button onClick={() => handleDelete(p.id)} className="px-3 py-2 rounded-lg bg-red-600/20 text-red-400 text-xs">
          ...
          <div className="space-y-4 max-h-[70vh] overflow-y-auto pr-2">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <Field 
                ...
                value={editing.it_sub_category || ""} 
                onChange={(v: string) => setEditing({ ...editing, it_sub_category: v })} 
            ...
            <Field 
              ...
              onChange={(v: string) => 
                setEditing({ ...editing, tags: v.split(",").map((t: string) => t.trim()).filter(Boolean) })
              } 
              placeholder="IT, WebDev" 
            />

            {/* ✅ FIXED */}
            <Field 
              label="Sky Gradient" 
              value={editing.sky_grad || ""} 
              onChange={(v: string) => setEditing({ ...editing, sky_grad: v })} 
              placeholder="from-sky-200 to-sky-400" 
            />

            {/* ✅ FIXED */}
            <Field 
              label="Hill Colors (hex, comma separated)" 
              value={editing.hill_colors?.join(",") || ""} 
              onChange={(v: string) => 
                setEditing({ ...editing, hill_colors: v.split(",").map((c: string) => c.trim()).filter(Boolean) })
              } 
              placeholder="#a3e635, #65a30d" 
            />
          </div>

          <div className="flex gap-2 mt-6">
            <button onClick={() => setEditing(null)} className="flex-1 py-2.5 rounded-xl bg-slate-800 text-slate-300">
              Cancel
            </button>
            <button 
              onClick={handleSave} 
              className="flex-1 py-2.5 rounded-xl bg-brand-yellow text-[#030c17] font-bold flex items-center justify-center gap-2"
            >
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-2xl max-h-[90vh] bg-[#0a1729] border border-slate-800 rounded-2xl p-6 overflow-y-auto">
        <button onClick={onClose} className="absolute top-4 right-4 p-2 rounded-full hover:bg-slate-800 text-slate-400">
          <X className="w-4 h-4" />
        </button>
        {children}
      </div>
...
function ProjectGalleryModal({ project, onClose, onUpdate }: GalleryModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-2xl bg-[#0a1729] border border-slate-800 rounded-2xl p-6">
          ...
          {localGallery.map((img) => (
            <div key={img.id} className="relative group rounded-xl overflow-hidden border border-slate-700 aspect-video">
              ...
              <button 
                ...
                className="absolute top-1 right-1 p-1.5 rounded-full bg-red-500/90 text-white opacity-0 group-hover:opacity-100"
              >
                <Trash2 className="w-3 h-3" />
              </button>
            </div>
        ...
        <button onClick={onClose} className="w-full py-2.5 rounded-xl bg-slate-800 text-slate-300">
          Done
        </button>
      </div>
    </div>
